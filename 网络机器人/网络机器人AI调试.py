...
response = re.get(TMDB_TOP_URL, timeout=60, headers=headers, verify=False)
...
for idx, movie in enumerate(movies):
    ...
    try:
        print(f"\n正在获取第 {idx + 1}/{len(movies)} 部电影详情: {movie['标题']}")

        # 添加延时，避免请求过快
        time.sleep(1)

        detail_response = re.get(movie['链接'], timeout=60, headers=headers, verify=False)
